...
Animation.prototype._createTokenGfx = function(scope) {
  ...
  var group = this.getGroup(scope);

  return svgAppendTo(parent, group);
};

Animation.prototype.getGroup = function(scope) {
  var plane = this._canvas.findPlane(scope.element),
      layer = plane.layer;

  var group = layer.querySelector('.animation-tokens');

  if (!group) {
    group = svgCreate('<g class="animation-tokens" />');
    svgAppendTo(
      group,
      layer
    );
